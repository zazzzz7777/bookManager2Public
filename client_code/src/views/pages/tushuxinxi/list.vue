	...
	<div class="list-page" :style='{}'>
		...
		<el-form :inline="true" :model="searchQuery" class="list_search">
			...
			<div class="search_btn_view">
				...
				<el-button class="add_btn" type="success" v-if="btnAuth('tushuxinxi','新增')" @click="addClick">新增</el-button>
			</div>
		</el-form>
			...
			<div class="data_box">
				<div class="data_view">
					<div class="data_item" v-for="(item,index) in list" :key="index" @click.stop="detailClick(item.id)" >
						<div class="data_img_box" v-if="item.tushutupian && item.tushutupian.substr(0,4)=='http'" @click.stop="preViewClick(item.tushutupian)">
							<el-image class="data_img" :src="item.tushutupian" fit="cover"></el-image>
						</div>
						<div class="data_img_box" v-else @click.stop="preViewClick($config.url+item.tushutupian.split(',')[0])">
							<el-image class="data_img" :src="item.tushutupian?$config.url + item.tushutupian.split(',')[0]:''"
								fit="cover"></el-image>
						</div>
						<div class="data_content">
							<div class="data_title"><span>{{item.tushumingcheng}}</span></div>
        ...
        <div class="hot_view">
            <div class="hot_title"><span>热门推荐</span></div>
            <div class="hot_list">
                <div class="hot" v-for="(item,index) in hotList" :key="index" @click="detailClick(item.id)">
                    <div class="hot_img_view"><el-image class="hot_img" fit="cover"
                        :src="$config.url + (item.tushutupian?item.tushutupian.split(',')[0]:'')"></el-image>
                    </div>
                    <div class="hot_content">
                        <div class="hot_text"><span>{{item.tushumingcheng}}</span></div>
                    </div>
                </div>
            </div>
        </div>
...
<script setup>
	...
	const addClick = () => {
		router.push('/index/tushuxinxiAdd')
	}
	//判断是否从个人中心跳转
	...
	const getList = () => {
		...
		if (categoryIndex.value != -1) {
			params.tushufenlei = categoryList.value[categoryIndex.value]
		}
